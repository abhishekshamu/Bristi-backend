...
async function run() {
    ...
    for (const post of BLOG_POSTS) {
        const existing = await BlogPost_1.BlogPostModel.findOne({ slug: post.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, '') });
        if (existing)
            continue;
        await BlogPost_1.BlogPostModel.create({
            ...post,
            status: 'published',
    ...
    for (const faq of FAQS) {
        const existing = await FAQ_1.FAQModel.findOne({ question: faq.question });
        if (existing)
            continue;
        await FAQ_1.FAQModel.create({ ...faq, isActive: true });
        createdFaqs++;
        console.log(`FAQ created: ${faq.question}`);
    }
    console.log(`FAQs seeded (${createdFaqs} created).`);
    const author = await User_1.UserModel.findOne({}).sort({ createdAt: 1 });
    if (author) {
        let createdPages = 0;
        for (const page of PAGES) {
            const existing = await Page_1.PageModel.findOne({ slug: page.slug });
            if (existing)
                continue;
            await Page_1.PageModel.create({
                ...page,
