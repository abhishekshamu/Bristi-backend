...
function block(heading, imageId, altText) {
    return {
        heading,
...
async function run() {
    ...
    const deleted = await HeroBlock_1.HeroBlockModel.deleteMany({});
    console.log(`Removed ${deleted.deletedCount} legacy hero block(s)`);
    const created = await HeroBlock_1.HeroBlockModel.create(DEMO_SET);
