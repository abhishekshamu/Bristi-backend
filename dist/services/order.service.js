...
class OrderService {
    constructor(orderRepo, productRepo, cartRepo, userRepo, couponRepo, inventoryRepo, settingsRepo, adminRepo, notificationService, emailService) {
    ...
    async createOrder(data) {
        ...
        if (!isGuest) {
            ...
            throw new exceptions_1.BadRequestException('Guest orders require a guest email');
        }
        if (!data.items || data.items.length === 0) {
            throw new exceptions_1.BadRequestException('Order must contain at least one item');
        }
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
            ...
            try {
                ...
                for (const itemData of data.items) {
                    ...
                    if (!product) {
                        throw new exceptions_1.NotFoundException(`Product not found: ${itemData.productId}`);
                    }
                    ...
                    if (itemData.variantId) {
                        variant = product.variants?.find((v) => String(v.id) === String(itemData.variantId));
                        if (!variant) {
                            throw new exceptions_1.BadRequestException(`Variant not found for ${product.name}`);
                        }
                    ...
                    if (product.trackQuantity) {
                        ...
                        const aggregateGuard = await Product_1.ProductModel.updateOne({ _id: product._id, stock: { $gte: itemData.quantity } }, { $inc: { stock: -itemData.quantity } }, { session }).exec();
                        if (aggregateGuard.modifiedCount === 0) {
                            throw new exceptions_1.BadRequestException(`Insufficient stock for ${product.name}`);
                        }
                        if (variant) {
                            const variantGuard = await Product_1.ProductModel.updateOne({ _id: product._id, 'variants.id': variant.id, 'variants.stock': { $gte: itemData.quantity } }, { $inc: { 'variants.$.stock': -itemData.quantity } }, { session }).exec();
                            if (variantGuard.modifiedCount === 0) {
                                throw new exceptions_1.BadRequestException(`Insufficient stock for ${product.name} (${variant.name})`);
                            }
                        }
                    }
                    ...
                    await this.inventoryRepo.applyOrderItem(product._id.toString(), itemData.variantId, itemData.quantity, undefined, session);
                }
                ...
                if (data.couponCode) {
                    ...
                    if (coupon && coupon.isValid) {
                        if (coupon.minimumPurchase && subtotal < coupon.minimumPurchase) {
                            throw new exceptions_1.BadRequestException(`Minimum purchase of $${coupon.minimumPurchase} required`);
                        }
                        if (coupon.perCustomerLimit && user) {
                            const customerUses = (coupon.customersUsed ?? []).filter((id) => String(id) === String(user._id)).length;
                        ...
                        if (!scoped.valid) {
                            throw new exceptions_1.BadRequestException(scoped.message || 'Coupon does not apply to these items');
                        }
                        ...
                        if (user) {
                            ...
                            await this.couponRepo.incrementUsage(coupon.code, undefined, session);
                        }
                    }
                }
                ...
                for (const itemData of data.items) {
                    await this.inventoryRepo.setHistoryOrderId(itemData.productId, itemData.variantId, order._id.toString(), session);
                }
                ...
                await this.emailService.sendOrderConfirmation(user ? user.email : data.guestEmail, {
                ...
                const duplicateOrderNumber = error?.code === 11000 && /orderNumber/.test(error?.message ?? '');
        ...
        throw new exceptions_1.BadRequestException('Could not generate a unique order number, please retry');
    }
    ...
    recordStatusHistory(order, status, note, actorName) {
        const entry = {
            status,
            note,
            changedBy: actorName,
    ...
    async updateOrderStatus(id, status, actorName, note) {
        const order = await this.orderRepo.findById(id);
        if (!order) {
            throw new exceptions_1.NotFoundException('Order not found');
        }
        if (order.status === 'cancelled' || order.status === 'refunded') {
            throw new exceptions_1.BadRequestException('Cancelled or refunded orders cannot change status');
        }
        if (status === order.status) {
            throw new exceptions_1.BadRequestException(`Order is already ${status}`);
        }
    ...
    async updatePaymentStatus(id, paymentStatus, paymentId, actorName) {
        const order = await this.orderRepo.findById(id);
        if (!order) {
            throw new exceptions_1.NotFoundException('Order not found');
        }
    ...
    async addTrackingInfo(id, trackingNumber, trackingUrl, actorName) {
        const order = await this.orderRepo.findById(id);
        if (!order) {
            throw new exceptions_1.NotFoundException('Order not found');
        }
    ...
    async cancelOrder(id, reason, actorName) {
        const order = await this.orderRepo.findById(id);
        if (!order) {
            throw new exceptions_1.NotFoundException('Order not found');
        }
        if (order.status !== 'pending' && order.status !== 'processing' && order.status !== 'confirmed') {
            throw new exceptions_1.BadRequestException('Only pending, confirmed or processing orders can be cancelled');
        }
    ...
    async refundOrder(id, reason, actorName) {
        const order = await this.orderRepo.findById(id);
        if (!order) {
            throw new exceptions_1.NotFoundException('Order not found');
        }
        if (order.status === 'cancelled' || order.status === 'refunded') {
            throw new exceptions_1.BadRequestException('This order cannot be refunded (already cancelled or refunded)');
        }
    ...
    async sendOrderEmail(id, type) {
        ...
        if (!email) {
            throw new exceptions_1.BadRequestException('No customer email available for this order');
        }
        ...
        return (await this.orderRepo.updateById(id, { emailHistory }));
    }
    async restoreStockForOrder(order, reason) {
        ...
        try {
            ...
            for (const item of order.items) {
                await Product_1.ProductModel.updateOne({ _id: item.productId, ...(item.variantId ? { 'variants.id': item.variantId } : {}) }, item.variantId
                    ? { $inc: { 'variants.$.stock': item.quantity, stock: item.quantity } }
                    : { $inc: { stock: item.quantity } }, { session }).exec();
                await this.inventoryRepo.restoreOrderItem(item.productId.toString(), item.variantId || undefined, item.quantity, order._id.toString(), reason, session);
            }
    ...
    generateOrderNumber(prefix = 'BRS', length = 8) {
        const date = new Date();
